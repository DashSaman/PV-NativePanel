...
ORDER BY enabled DESC, sort_order ASC, name ASC`)
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&plan.NoExpiry, &startPolicy, &resetStrategy, &plan.ResetCustomDays, &plan.ConcurrencyLimit, &plan.UniqueIPLimit,
...
INSERT INTO pvnaive.plans (
    ...
    reset_custom_days, concurrency_limit, unique_ip_limit, enabled, sort_order, default_group_id
) VALUES (
    $1::uuid, $2, $3, 'active', 'naive', $4, $5,
    0, 'USD', $6, $7, $8, NULLIF($9,0), $10, $11, $12, $13, NULLIF($14,'')::uuid
)
RETURNING id::text, name, quota_bytes,
		...
		plan.NoExpiry, string(plan.ResetStrategy), plan.ResetCustomDays, plan.ConcurrencyLimit, plan.UniqueIPLimit, plan.Enabled,
