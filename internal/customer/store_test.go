...
func TestPostgresStoreCreatesDirectUserAndServiceTerm(t *testing.T) {
	...
	conn := &customerScriptConn{queries: []customerScriptQuery{
		...
		{contains: "INSERT INTO pvnaive.service_terms", columns: []string{"id", "tenant_id", "user_id", "quota_bytes", "concurrency_limit", "unique_ip_limit", "duration_seconds", "start_policy", "purchased_at", "starts_at", "first_connected_at", "expires_at", "state", "accounting_baseline_state", "accounting_baseline_source", "accounting_baseline_cutoff_at", "accounting_baseline_upload_bytes", "accounting_baseline_download_bytes", "revision"}, values: []driver.Value{"term-1", "tenant-direct", "user-1", quota, int64(concurrency), nil, int64(2592000), "on_creation", now, now, nil, expires, "active", "known", "fresh_managed_term", now, int64(0), int64(0), int64(1)}},
	...
	term, err := store.CreateServiceTermTx(context.Background(), tx, CreateServiceTermRecord{
		TenantID: tenantID, UserID: user.ID, QuotaBytes: &quota, ConcurrencyLimit: &concurrency, DurationSeconds: 2592000,
	...
	if term.ConcurrencyLimit == nil || *term.ConcurrencyLimit != concurrency {
		t.Fatalf("ConcurrencyLimit = %v, want %d", term.ConcurrencyLimit, concurrency)
	}
