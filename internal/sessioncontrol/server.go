...
func NewHandler(k killer) http.Handler {
	...
	mux.HandleFunc("/v1/sessions/kill", func(w http.ResponseWriter, r *http.Request) {
		...
		if wire.RuntimeCredentialID == "" || wire.NodeID == "" || wire.BootID == "" || wire.SessionID == "" {
			http.Error(w, "incomplete session tuple", http.StatusBadRequest)
			return
		}

		result, err := k.Kill(sessionkill.Key{
			RuntimeCredentialID: wire.RuntimeCredentialID,
			NodeID:              wire.NodeID,
			BootID:              wire.BootID,
