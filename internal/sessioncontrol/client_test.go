...
func TestClientKillSendsExactTupleAndReturnsResult(t *testing.T) {
	...
	if !result.Found || !result.Killed {
		t.Fatalf("expected found=true killed=true, got %+v", result)
	}
	if seen.RuntimeCredentialID != key.RuntimeCredentialID || seen.NodeID != key.NodeID || seen.BootID != key.BootID || seen.SessionID != key.SessionID {
...
func TestClientKillIdempotentReturnsFoundNotKilled(t *testing.T) {
	...
	if !result.Found || result.Killed {
		t.Fatalf("expected found=true killed=false, got %+v", result)
	}
}
...
func TestClientKillNotFoundReturnsBothFalse(t *testing.T) {
	...
	if result.Found || result.Killed {
		t.Fatalf("expected both false, got %+v", result)
	}
}

func TestClientKillServerDownReturnsError(t *testing.T) {
	client := &Client{httpClient: &http.Client{Transport: &http.Transport{DialContext: func(_ context.Context, _, _ string) (net.Conn, error) {
		return nil, &net.OpError{Op: "dial", Net: "unix", Err: errors.New("connection refused")}
	}}, Timeout: 2 * time.Second}}
	if _, err := client.Kill(context.Background(), testKey()); err == nil {
...
func TestClientKillRejectsTrailingJSON(t *testing.T) {
	...
	defer server.Close()
	if _, err := testClient(server).Kill(context.Background(), testKey()); err == nil {
