...
func TestClientKillSendsExactTupleAndReturnsResult(t *testing.T) {
	...
	if !result.Found || !result.Killed { t.Fatalf("expected found=true killed=true, got found=%v killed=%v", result.Found, result.Killed) }
	if seen.RuntimeCredentialID != key.RuntimeCredentialID || seen.SessionID != key.SessionID { t.Fatalf("wrong tuple sent: %+v", seen) }
}
...
func TestClientKillIdempotentReturnsFoundNotKilled(t *testing.T) {
	...
	defer server.Close()
	transport := &http.Transport{DialContext: func(ctx context.Context, _, _ string) (net.Conn, error) { var dialer net.Dialer; return dialer.DialContext(ctx, "tcp", server.Listener.Addr().String()) }}
	client := &Client{httpClient: &http.Client{Transport: transport}}
	result, err := client.Kill(context.Background(), testKey())
	if err != nil { t.Fatalf("Kill: %v", err) }
	if !result.Found || result.Killed { t.Fatalf("expected found=true killed=false, got found=%v killed=%v", result.Found, result.Killed) }
}

func TestClientKillNotFoundReturnsBothFalse(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) { w.Header().Set("Content-Type", "application/json"); _ = json.NewEncoder(w).Encode(KillResult{Found: false, Killed: false}) }))
	defer server.Close()
	transport := &http.Transport{DialContext: func(ctx context.Context, _, _ string) (net.Conn, error) { var dialer net.Dialer; return dialer.DialContext(ctx, "tcp", server.Listener.Addr().String()) }}
	client := &Client{httpClient: &http.Client{Transport: transport}}
	result, err := client.Kill(context.Background(), testKey())
	if err != nil { t.Fatalf("Kill: %v", err) }
	if result.Found || result.Killed { t.Fatalf("expected found=false killed=false, got found=%v killed=%v", result.Found, result.Killed) }
}
...
func TestClientKillServerDownReturnsError(t *testing.T) {
	...
	if err == nil { t.Fatal("expected error for unreachable socket") }
}

func TestClientKillRejectsTrailingJSON(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) { w.Header().Set("Content-Type", "application/json"); _, _ = w.Write([]byte(`{"found":true,"killed":true}{"extra":"data"}`)) }))
	defer server.Close()
	transport := &http.Transport{DialContext: func(ctx context.Context, _, _ string) (net.Conn, error) { var dialer net.Dialer; return dialer.DialContext(ctx, "tcp", server.Listener.Addr().String()) }}
	client := &Client{httpClient: &http.Client{Transport: transport}}
	_, err := client.Kill(context.Background(), testKey())
	if err == nil { t.Fatal("expected error for trailing JSON") }
}
