...
func TestTelegramChannelSendsSanitizedPayload(t *testing.T) {
	...
	if path != "/botbot-secret-token/sendMessage" {
		t.Fatalf("path=%q", path)
	}
	for _, secret := range []string{"leaked-value", "hunter2"} {
		if strings.Contains(body, secret) {
			t.Fatalf("Telegram payload leaked %q: %s", secret, body)
		}
	}
	if !strings.Contains(body, "[REDACTED]") {
		t.Fatalf("sanitized marker absent: %s", body)
	}
}
...
func TestTelegramFailureDoesNotExposeBotToken(t *testing.T) {
	...
	if err == nil {
		t.Fatal("non-2xx Telegram response must fail")
	}
