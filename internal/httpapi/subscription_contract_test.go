...
func TestBrowserOpeningMachineEndpointRedirectsToAccountPage(t *testing.T) {
	handler, token := subscriptionContractFixture(t, subscription.Record{})
	req := httptest.NewRequest(http.MethodGet, "/sub/"+token, nil)
	...
	if got := res.Header().Get("Location"); got != "/s/"+token {
		t.Fatalf("location=%q", got)
	}
}

func TestBrowserWithExplicitFamilyOverrideStillGetsRawPayload(t *testing.T) {
	handler, token := subscriptionContractFixture(t, subscription.Record{})
	req := httptest.NewRequest(http.MethodGet, "/sub/"+token+"?family=naive", nil)
	...
	if strings.Contains(strings.ToLower(res.Body.String()), "<!doctype html") {
		t.Fatal("family override from a browser must serve the raw payload")
	}
}

func TestClientUserAgentsKeepRawMachineDelivery(t *testing.T) {
	handler, token := subscriptionContractFixture(t, subscription.Record{})
	for _, ua := range []string{
		"Karing/1.1.5.560",
		"SFA/1.10.0 (Android 14; sing-box 1.10.0)",
		"NekoBox/1.3.2 (Android)",
		"NekoRay/2.6 (Windows)",
		"v2rayNG/1.8.23",
		"curl/8.5.0",
		"",
		...
		handler.ServeHTTP(res, req)
		if res.Code != http.StatusOK {
			t.Fatalf("ua=%q status=%d body=%s", ua, res.Code, res.Body.String())
		}
		if strings.Contains(strings.ToLower(res.Body.String()), "<!doctype html") {
			t.Fatalf("ua=%q received HTML", ua)
		}
	}
}
