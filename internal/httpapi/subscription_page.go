...
@media(max-width:520px){.account-nav{align-items:flex-start}.nav-tools{align-items:flex-end;flex-direction:column}.hero-copy h1{font-size:24px}.service-grid{grid-template-columns:1fr 1fr}.service-card{min-height:78px;padding:12px}.service-card strong{font-size:14px}.service-strip{grid-template-columns:1fr 1fr}.qrgrid{grid-template-columns:1fr 1fr}.connect-card,.qr-panel,.help-card{padding:13px}.connection-item-head{align-items:flex-start;flex-direction:column}.actions{display:grid;grid-template-columns:1fr}.actions .btn{width:100%}.brand strong{font-size:16px}.help-clients{grid-template-columns:1fr}}
...
<section class="help-card"><h2>{{.M.HelpTitle}}</h2><p class="help-intro">{{.M.HelpIntro}}</p><div class="steps"><div class="step"><b>1</b><span>{{.M.StepOne}}</span></div><div class="step"><b>2</b><span>{{.M.StepTwo}}</span></div><div class="step"><b>3</b><span>{{.M.StepThree}}</span></div></div><div class="help-clients">{{range .M.HelpClients}}<article class="help-client"><h3>{{.Name}}</h3><p>{{.Summary}}</p><ol>{{range .Steps}}<li>{{.}}</li>{{end}}</ol></article>{{end}}</div><div class="help-tips"><b>{{.M.HelpTipsTitle}}</b><ul>{{range .M.HelpTips}}<li>{{.}}</li>{{end}}</ul></div></section>
