...
func (s *Service) ResolveProfile(ctx context.Context, rawToken, host string) (Profile, error) {
        ...
        // Remark: clients (Karing, NekoBox, v2rayNG, …) show the URI fragment
        // as the profile/server label after a scan or import. Without it the
        // entry lands as a bare host string; with it the account reads
        // "PVNaive-<username>" on every client automatically.
        if parsed, parseErr := url.Parse(uri); parseErr == nil {
                parsed.Fragment = "PVNaive-" + record.Username
                uri = parsed.String()
        }
