...
func RequiredEnvironmentCheck(names []string, getenv func(string) string) Check {
	return CheckFunc{Name: "required-env", Func: func(context.Context) CheckResult {
		...
		return CheckResult{Name: "required-env", Status: Fail, Detail: "missing: " + strings.Join(missing, ",")}
	}}
}

func ServiceCheck(unit string) Check {
	return CheckFunc{Name: "service:" + unit, Func: func(ctx context.Context) CheckResult {
		command := exec.CommandContext(ctx, "systemctl", "is-active", unit)
		output, err := command.Output()
		if err != nil {
			return CheckResult{Name: "service:" + unit, Status: Fail, Detail: "not active"}
		}
		if strings.TrimSpace(string(output)) != "active" {
			return CheckResult{Name: "service:" + unit, Status: Fail, Detail: "not active"}
		}
		return CheckResult{Name: "service:" + unit, Status: Pass, Detail: "active"}
	}}
}
...
func RuntimeSocketCheck(path string) Check {
	return CheckFunc{Name: "runtime-socket", Func: func(context.Context) CheckResult {
		info, err := os.Stat(path)
		if err != nil {
			return CheckResult{Name: "runtime-socket", Status: Fail, Detail: "socket missing"}
		}
		if info.Mode()&os.ModeSocket == 0 {
			return CheckResult{Name: "runtime-socket", Status: Fail, Detail: "path is not a Unix socket"}
		}
		if info.Mode().Perm()&0o007 != 0 {
			return CheckResult{Name: "runtime-socket", Status: Warn, Detail: fmt.Sprintf("socket mode %04o is accessible to other users", info.Mode().Perm())}
		}
		return CheckResult{Name: "runtime-socket", Status: Pass, Detail: fmt.Sprintf("socket mode %04o", info.Mode().Perm())}
	}}
}
