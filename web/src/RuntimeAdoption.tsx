...
function AdoptionForm({ credential, onDone, onClose }: { credential: RuntimeCredential; onDone: () => Promise<void>; onClose: () => void }) {
  ...
  return <form className="product-form" onSubmit={submit}>
    ...
    <div className="modal-actions"><button type="button" className="button-secondary" onClick={onClose}>انصراف</button><button className="primary-action" disabled={busy}>{busy ? "در حال اتصال…" : "اتصال به Customer Product"}</button></div>
  </form>;
}
...
export function RuntimeAdoption() {
  ...
  return <main className="product-page">
    <header className="product-hero"><div><p className="eyebrow">Owner-only · Legacy adoption</p><h1>اکانت‌های قدیمی Runtime</h1><p>این صفحه فقط credentialهای فعال قدیمی را که هنوز Customer Product ندارند نشان می‌دهد. بعد از اتصال، مدیریت حجم/تاریخ/تمدید از صفحه کاربران انجام می‌شود.</p></div><div className="hero-actions"><a className="button-secondary" href="/panel/#/customers">بازگشت به کاربران</a><button onClick={() => void refresh()}>↻ بروزرسانی</button></div></header>
    <section className="product-stats"><article><span>Runtime کل</span><strong>{credentials.length.toLocaleString("fa-IR")}</strong></article><article><span>نیازمند اتصال</span><strong>{unmanaged.length.toLocaleString("fa-IR")}</strong></article><article><span>متصل‌شده</span><strong>{managedIDs.size.toLocaleString("fa-IR")}</strong></article><article><span>دسترسی</span><strong>Owner-only</strong></article></section>
    {message && <div className="product-message">{message}</div>}
    <section className="product-card"><div className="product-card-head"><div><p className="eyebrow">Unmanaged runtime</p><h2>اکانت‌های نیازمند تنظیم</h2></div></div><div className="catalog-list">{loading && <p className="muted">در حال بارگذاری…</p>}{!loading && unmanaged.length === 0 && <p className="muted">همه credentialهای فعال Runtime به Customer Product متصل هستند.</p>}{unmanaged.map((credential) => <div className="catalog-item" key={credential.id}><div><strong>{credential.username}</strong><small>{credential.origin} · {credential.id.slice(0, 12)} · revision {credential.revision}</small></div><button className="primary-action" onClick={() => setDialog({ credential })}>تنظیم حجم و اعتبار</button></div>)}</div></section>
