...
describe("auth client", () => {
  ...
  it("submits login credentials without adding CSRF before authentication", async () => {
    const fetcher = vi.fn(async () => new Response(JSON.stringify({ status: "authenticated", role: "owner" }), {
      status: 200,
  ...
  it("binds logout to the browser CSRF token", async () => {
    const fetcher = vi.fn(async () => new Response(JSON.stringify({ status: "logged_out" }), {
      status: 200,
      headers: { "Content-Type": "application/json" },
    }));
