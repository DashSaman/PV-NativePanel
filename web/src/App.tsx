...
import { AccountingLive } from "./AccountingLive";
...
type View = "dashboard" | "customers" | "accounting-live" | "runtime-naive";
const metrics = [["مدیریت مشتری","فعال"],["Subscription / QR","فعال"],["Accounting دقیق","فعال"],["Runtime","قابل مدیریت"]];

export function currentView(hash = window.location.hash): View {
  if (hash === "#/customers") return "customers";
  if (hash === "#/accounting") return "accounting-live";
...
export function App() {
  ...
  if (view === "accounting-live" && owner) return <div className="shell"><Sidebar principal={principal} signOut={signOut}/><AccountingLive/><MobileNav owner={owner} signOut={signOut}/></div>;
  ...
  return <div className="shell"><Sidebar principal={principal} signOut={signOut}/><main><header><div><p className="eyebrow">Standalone control plane</p><h1>داشبورد PVNaive</h1></div><span className="badge">ورود امن فعال</span></header><section className="notice" role="status">{principal.display_name} · {principal.email} · نقش: {principal.role}</section><section className="metrics" aria-label="شاخص‌ها">{metrics.map(([label,value]) => <article key={label}><span>{label}</span><strong>{value}</strong></article>)}</section><section className="panel"><div><p className="eyebrow">Customer service</p><h2>مدیریت اکانت‌های Naive</h2><p>{owner ? "اکانت را با حجم، تاریخ انقضا، لینک اشتراک و QR بسازید؛ مصرف دقیق و Online هم در نمای مصرف زنده در دسترس است." : "مدیریت مشتری فقط برای Owner در دسترس است."}</p></div>{owner ? <div className="header-actions"><a className="button-secondary" href="/panel/#/customers">مدیریت مشتریان</a><a className="button-secondary" href="/panel/#/accounting">مصرف زنده</a></div> : <button disabled>فقط Owner</button>}</section></main><MobileNav owner={owner} signOut={signOut}/></div>;
}