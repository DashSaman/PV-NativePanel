...
func run() error {
	...
	// Abandoned-reservation reconciler: drives the 0022 release path so a
	// crashed claim (unclean restart between quota reservation and its
	// settling telemetry event) never bricks a user's connectivity.
	reconcileConfig, err := accountingReconcileConfigFromEnv(os.Getenv)
	if err != nil {
		return fmt.Errorf("accounting reconcile configuration: %w", err)
	}
	log.Printf(
		"PVNaive accounting reconcile loop enabled: tick=%s stale_window=%s batch=%d",
		reconcileConfig.Tick, reconcileConfig.StaleWindow, reconcileConfig.BatchLimit,
	)
	go runAccountingReconcileLoop(runCtx, accountingReconcileDBExecutor{db: db}, reconcileConfig, log.Printf)
