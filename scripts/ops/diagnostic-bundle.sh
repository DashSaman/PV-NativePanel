...
for unit in \
  ...
  pvnaive-restore-drill.timer; do
...
for unit in pvnaive-api.service pvnaive-runtime-agent.service pvnaive-telemetry-agent.service caddy-naive.service; do
  echo "===== $unit ====="
...

{
  ...
  stat -c '%n mode=%a owner=%U group=%G type=%F' /run/pvnaive/runtime-agent.sock /run/pvnaive/accounting.sock 2>/dev/null || true
  echo '=== listeners ==='
...
if [[ -f /opt/pvnaive/release/CURRENT ]]; then
  redact < /opt/pvnaive/release/CURRENT >"$tmp/release.txt"
fi
