...
stage_id="S06-ACCOUNTING-PREFLIGHT"
...
[[ ${EUID} -eq 0 ]] || { echo 'ERROR: run S06 accounting preflight as root' >&2; exit 1; }

echo "=== ${stage_id} ==="
echo "UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "HOSTNAME=$(hostname)"
echo "S06_ACCOUNTING_BASE_SCHEMA=${S06_ACCOUNTING_BASE_SCHEMA}"
echo "S06_ACCOUNTING_TARGET_SCHEMA=${S06_ACCOUNTING_TARGET_SCHEMA}"
...
if [[ -x "${caddy_bin}" ]]; then
  ACCOUNTING_CADDY_SHA256="$(sha256sum "${caddy_bin}" | awk '{print $1}')"
fi
ACCOUNTING_CADDY_MODULE="false"
if [[ -x "${caddy_bin}" ]] && "${caddy_bin}" list-modules 2>/dev/null | grep -qF 'pvnaive_accounting'; then
  ACCOUNTING_CADDY_MODULE="true"
fi
echo "ACCOUNTING_CADDY_SHA256=${ACCOUNTING_CADDY_SHA256:-unavailable}"
echo "ACCOUNTING_CADDY_MODULE=${ACCOUNTING_CADDY_MODULE}"
...
if [[ -r "${db_env}" ]]; then
  ...
  [[ "${actual}" == "${S06_ACCOUNTING_BASE_SCHEMA}" ]] && pass DB_SCHEMA_BASELINE || failcheck DB_SCHEMA_BASELINE
  [[ "${expected}" == "${S06_ACCOUNTING_BASE_SCHEMA}" ]] && pass DB_SCHEMA_EXPECTATION || failcheck DB_SCHEMA_EXPECTATION
else
  failcheck DB_ENV
fi
