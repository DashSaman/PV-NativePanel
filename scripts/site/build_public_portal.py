...
ALLOWED_MEDIA_MIME = {"video/mp4", "video/webm", "audio/mpeg", "audio/mp4", "audio/ogg", "application/pdf"}
...
def validate_local_path(value: str) -> str:
    ...
    if ".." in Path(value).parts or "." in Path(value).parts:
        raise ValueError(f"invalid local path: {value}")
...
def load_and_validate_media(path: Path = DATA_ROOT / "media.json"):
    ...
    ids, slugs = set(), set()
    required = ["id", "slug", "kind", "title", "summary", "source_name", "source_domain", "source_url", "attribution", "qualities"]
    for item in items:
        for key in required:
        ...
        if item["kind"] not in {"video", "audio", "pdf", "document"}:
        ...
        for quality in item["qualities"]:
            if quality.get("mime") not in ALLOWED_MEDIA_MIME:
                raise ValueError(f"unsupported media MIME: {quality.get('mime')}")
...
def load_portal():
    payload = load_json(DATA_ROOT / "portal.json")
...
def link(href: str, label: str, css: str = "") -> str:
    ...
    return f'<a href="{esc(href)}"{class_attr}{external}>{esc(label)}</a>'
...
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#101b27"><meta name="description" content="{disclosure}"><link rel="stylesheet" href="/assets/site.css"><link rel="icon" href="/assets/news-mark.svg" type="image/svg+xml"><title>{esc(title)} | {esc(portal['name'])}</title></head>
<body class="portal-page">
<header class="portal-masthead"><div class="shell portal-masthead__inner"><a class="wordmark" href="/"><img src="/assets/news-mark.svg" width="44" height="44" alt=""><span><strong>{esc(portal['name'])}</strong><small>{esc(portal['tagline'])}</small></span></a><nav class="portal-nav" aria-label="ناوبری اصلی">{nav_html(portal)}</nav></div></header>
<main class="shell portal-main"><div class="portal-breadcrumb"><a href="/">خانه</a>{breadcrumb}</div>{body}</main>
...
def build_news(portal, articles, out_root: Path):
    ...
    for article in articles.get("articles", []):
        ...
        body = f'<article class="portal-detail"><div class="story-meta"><span>{esc(article.get("category"))}</span><span>{esc(article.get("published_label"))}</span></div><h1>{esc(article["title"])}</h1><p class="portal-lead">{esc(article["summary"])}</p><div class="portal-source-box"><strong>منبع اصلی</strong>{link(article["source_url"], "مشاهده مطلب در منبع اصلی")}</div><div class="portal-related">{link("/news/", "بازگشت به آرشیو خبرها")} · {link("/sources/", "فهرست منابع")}</div></article>'
...
def build_media_kind(portal, media, out_root: Path, kind: str):
    dirname, label = (("videos", "ویدئو") if kind == "video" else ("audio", "صوت"))
    items = [item for item in media["items"] if item["kind"] == kind]
    cards = []
    for item in items:
        cards.append(card(item["title"], item["summary"], f"/{dirname}/{item['slug']}.html", item.get("published_label", "")))
        ...
        source_tags = "".join(f'<source src="{esc(safe_url(src))}" type="{esc(mime)}">' for src, mime in player_sources(item))
        rows = []
        for quality in item["qualities"]:
            size = human_bytes(quality.get("bytes")) or "اندازه در منبع تثبیت نشده"
            if item.get("mirror_allowed") and quality.get("local_path"):
                receive = f'<a href="/media/{esc(quality["local_path"])}" download>دانلود از این سایت</a> · {link(quality["url"], "منبع مستقیم")}'
            else:
        ...
        source_domain = f'<span class="source-domain">{esc(item["source_domain"])}</span>'
        body = f'<article class="portal-detail media-detail"><div class="story-meta"><span>{esc(label)}</span><span>{esc(item.get("published_label"))}</span></div><h1>{esc(item["title"])}</h1><p class="portal-lead">{esc(item["summary"])}</p><div class="media-player"><{tag}{attrs}>{source_tags}مرورگر شما پخش این فایل را پشتیبانی نمی‌کند.</{tag}></div><div class="portal-source-box"><strong>انتساب و مجوز</strong>{source_domain}<p>{esc(item["attribution"])}</p><p>{esc(item.get("rights_note"))}</p>{link(item["source_url"], "صفحه منبع")}</div><div class="download-table"><table><thead><tr><th>کیفیت</th><th>فرمت</th><th>حجم</th><th>دریافت</th></tr></thead><tbody>{"".join(rows)}</tbody></table></div><div class="portal-related">{link(f"/{dirname}/", f"بازگشت به آرشیو {label}")} · {link("/downloads/", "همه دانلودها")} · {link("/sources/", "منابع")}</div></article>'
...
def build_galleries(portal, galleries, out_root: Path):
    ...
    for gallery in galleries.get("galleries", []):
        cards.append(card(gallery["title"], gallery["summary"], f"/gallery/{gallery['slug']}.html", gallery.get("published_label", "")))
...
def build_downloads(portal, media, out_root: Path):
    ...
    for item in media["items"]:
        ...
        for quality in item["qualities"]:
            ...
            if item.get("mirror_allowed") and quality.get("local_path"):
                receive = f'<a href="/media/{esc(quality["local_path"])}" download>دانلود محلی</a> · {link(quality["url"], "نسخه منبع")}'
            else:
                receive = link(quality["url"], "دانلود از منبع")
            rows.append(f'<tr><td>{link(detail_href, item["title"])}</td><td>{esc(quality["mime"])}</td><td>{esc(size)}</td><td>{esc(item["source_name"])}</td><td>{receive}</td></tr>')
...
def build_sources(portal, articles, media, out_root: Path):
    sources = {source["domain"]: (source["name"], source["homepage"]) for source in articles.get("sources", [])}
    for item in media["items"]:
