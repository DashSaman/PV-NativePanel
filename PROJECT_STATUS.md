...
- Task16: draft PR #81, exact GitHub head `3c4310335ab4907d28bac995bba1be3545e14f6e`; PR body is stale and base is historical `0b921abe...`. Dedicated Task16/Exact Accounting/Pinned Forwardproxy workflows are green, but generic CI run `33678134360` fails in `database`.
- Latest CI failure details: the generic database job reaches `schema version=21, want=20` after earlier tests pass; the run also records expected guardrail errors (append-only DELETE rejection, invalid refresh session, RLS denial) as test evidence, not production incidents.
...
- The active executable lane in this run was `pv-worker-main`; `pv-primary` was inactive under the one-active-host limit.
...
- A real validation attempt on the worker failed at `tests/db/auth_refresh_reuse_test.sh:51` with `unexpected EOF while looking for matching ')'`; `git diff --check` did not fail before that syntax error.
- No fresh worker completion was creditable.

## Immediate execution order

1. Keep #64 and #81 draft / DO NOT MERGE.
2. Reconcile the Task16 branch to one exact published head, then rerun the generic database gate after the narrow schema21/latest-schema fixture correction; preserve Task15 schema20-specific tests.
3. On the first clean executable development worker, run the final exact-head Task13 HTTP/1.1 + HTTP/2 rehearsal with PostgreSQL 18 and compatible Go/jq tooling.
