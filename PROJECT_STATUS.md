...
- Current `main`: `287cb5880747d83c4d82aed3994ad37628bee782`.
- No status rows are visible for this exact `main` head; post-merge CI is not proven.
...
- Task16: draft PR #81, actual GitHub head `3c4310335ab4907d28bac995bba1be3545e14f6e`; the PR body claims an older head and is stale. Dedicated Task16/Exact Accounting/Pinned Forwardproxy gates are green; generic CI run `33678134360` failed in `database`. Failed jobs were re-run for fresh evidence; result is pending.
...
Fresh read-only observation on `pv-primary` at 2026-09-05 01:37 Asia/Tehran:
...
- readiness: HTTP 200, `db=ok`, `schema=ok`, `ready=true`;
- liveness: HTTP 200;
- SNI-correct local HTTPS probe for `namir.softarg.ir`: HTTP 200.

No Production mutation, deploy, restart, reload, migration, DB write or credential change was performed in this run.
