...
- Exact-head #64 workflows on `5bc42d8...`: WS1 Exact Accounting run `33557107038` is **SUCCESS**; CI run `33557107036` and WS1 Pinned Forwardproxy run `33557107045` are running on the exact new head and must finish green before merge.
...
- The prior Task13 head passed PostgreSQL18 database gates, Go/vet/tests, Web tests/build, S04/S04R rehearsal and the R1 bundle job. The new `5bc42d8...` head is not credited until its exact-head CI finishes green.
...
1. Keep PR #64 draft despite the prior exact head being green; the remaining authorization and live protocol/accounting proof still gates merge.
2. Require the published PostgreSQL18 DB-integrated ownership/IDOR/CSRF rehearsal on `5bc42d8...` to finish green; do not infer it from local tests.
