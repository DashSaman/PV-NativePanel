...
- Current roadmap PR: **draft PR #64**, branch `lead/task13-reconstruct-62573fee`, exact head `9a863258455473605a370f7ad4964043a0df92a1`.
- Exact-head #64 workflows for the new API publication are running; do not reuse green results from the prior head.
...
- New API increment wires ownership-checked exact-session kill through the authenticated customer/RLS read model. Exact runtime credential/node/boot/session tuple is selected from trusted active-session state; request tuple forgery is not accepted; no credential rotation/revoke is performed. Worker RED→GREEN plus focused race/full Go/diff checks passed.
...
2. Continue Task13 on `TrPaqet` with the narrow socket service/group permission proof, then stronger handler-level ownership/IDOR/CSRF failure tests and UI.
