...
[[ "${source_schema}" == "22" ]] || { echo "ERROR: backup source expected latest schema 22, got ${source_schema}" >&2; exit 1; }
...
grep -Fq '"schema_version": 22,' "${backup_dir}/metadata.json"
...
[[ "${restored_schema}" == 22 ]] || { echo "ERROR: restored schema expected 22, got ${restored_schema}" >&2; exit 1; }
