...
[[ "${source_schema}" == "7" ]] || { echo "ERROR: backup source expected latest schema 7, got ${source_schema}" >&2; exit 1; }
...
grep -Fq '"schema_version": 7,' "${backup_dir}/metadata.json"
...
[[ "${restored_schema}" == 7 ]] || { echo "ERROR: restored schema expected 7, got ${restored_schema}" >&2; exit 1; }
