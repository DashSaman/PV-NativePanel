...
[[ "${source_schema}" == "15" ]] || { echo "ERROR: backup source expected latest schema 15, got ${source_schema}" >&2; exit 1; }
...
grep -Fq '"schema_version": 15,' "${backup_dir}/metadata.json"
...
[[ "${restored_schema}" == 15 ]] || { echo "ERROR: restored schema expected 15, got ${restored_schema}" >&2; exit 1; }
