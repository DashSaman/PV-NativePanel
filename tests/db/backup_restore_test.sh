...
[[ "${source_schema}" == "16" ]] || { echo "ERROR: backup source expected latest schema 16, got ${source_schema}" >&2; exit 1; }
...
grep -Fq '"schema_version": 16,' "${backup_dir}/metadata.json"
...
[[ "${restored_schema}" == 16 ]] || { echo "ERROR: restored schema expected 16, got ${restored_schema}" >&2; exit 1; }
