...
[[ "${source_schema}" == "10" ]] || { echo "ERROR: backup source expected latest schema 10, got ${source_schema}" >&2; exit 1; }
...
grep -Fq '"schema_version": 10,' "${backup_dir}/metadata.json"
...
[[ "${restored_schema}" == 10 ]] || { echo "ERROR: restored schema expected 10, got ${restored_schema}" >&2; exit 1; }
