...
[[ "${source_schema}" == "11" ]] || { echo "ERROR: backup source expected latest schema 11, got ${source_schema}" >&2; exit 1; }
...
grep -Fq '"schema_version": 11,' "${backup_dir}/metadata.json"
...
[[ "${restored_schema}" == 11 ]] || { echo "ERROR: restored schema expected 11, got ${restored_schema}" >&2; exit 1; }
