...
[[ "${source_schema}" == "19" ]] || { echo "ERROR: backup source expected latest schema 19, got ${source_schema}" >&2; exit 1; }
...
grep -Fq '"schema_version": 19,' "${backup_dir}/metadata.json"
...
[[ "${restored_schema}" == 19 ]] || { echo "ERROR: restored schema expected 19, got ${restored_schema}" >&2; exit 1; }
