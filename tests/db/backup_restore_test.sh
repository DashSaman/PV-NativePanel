...
[[ "${source_schema}" == "17" ]] || { echo "ERROR: backup source expected latest schema 17, got ${source_schema}" >&2; exit 1; }
...
grep -Fq '"schema_version": 17,' "${backup_dir}/metadata.json"
...
[[ "${restored_schema}" == 17 ]] || { echo "ERROR: restored schema expected 17, got ${restored_schema}" >&2; exit 1; }
