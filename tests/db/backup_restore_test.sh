...
[[ "${source_schema}" == "20" ]] || { echo "ERROR: backup source expected latest schema 20, got ${source_schema}" >&2; exit 1; }
...
grep -Fq '"schema_version": 20,' "${backup_dir}/metadata.json"
...
[[ "${restored_schema}" == 20 ]] || { echo "ERROR: restored schema expected 20, got ${restored_schema}" >&2; exit 1; }
