...
[[ "${source_schema}" == "9" ]] || { echo "ERROR: backup source expected latest schema 9, got ${source_schema}" >&2; exit 1; }
...
grep -Fq '"schema_version": 9,' "${backup_dir}/metadata.json"
...
[[ "${restored_schema}" == 9 ]] || { echo "ERROR: restored schema expected 9, got ${restored_schema}" >&2; exit 1; }
