...
[[ "${source_schema}" == "14" ]] || { echo "ERROR: backup source expected latest schema 14, got ${source_schema}" >&2; exit 1; }
...
grep -Fq '"schema_version": 14,' "${backup_dir}/metadata.json"
...
[[ "${restored_schema}" == 14 ]] || { echo "ERROR: restored schema expected 14, got ${restored_schema}" >&2; exit 1; }
