...
# the migration being reverted. For a v5 -> v4 rollback, that backup is schema 4.
...
v4_migrations="${temp_root}/migrations-v4"
mkdir -p "${v4_migrations}"
cp "${repo_root}"/db/migrations/0001_* "${v4_migrations}/"
cp "${repo_root}"/db/migrations/0002_* "${v4_migrations}/"
cp "${repo_root}"/db/migrations/0003_* "${v4_migrations}/"
cp "${repo_root}"/db/migrations/0004_* "${v4_migrations}/"
(
  cd "${v4_migrations}"
  sha256sum *.sql > SHA256SUMS
)
PVNAIVE_DB_NAME="${rollback_db}" PVNAIVE_MIGRATIONS_DIR="${v4_migrations}" \
  "${repo_root}/scripts/db/migrate.sh" >/dev/null
...
[[ "${rollback_schema_before}" == "4" ]] || { echo "ERROR: rollback fixture did not reach schema 4" >&2; exit 1; }
...
grep -Fq '"schema_version": 4,' "$(dirname -- "${rollback_backup_file}")/metadata.json"
...
[[ "${rollback_schema_migrated}" == "5" ]] || { echo "ERROR: rollback fixture did not reach schema 5" >&2; exit 1; }
...
grep -Fqx 'PVNAIVE_SCHEMA_VERSION=4' <<< "${rollback_output}"
...
[[ "${rollback_schema_after}" == "4" ]] || { echo "ERROR: production rollback did not return schema 5 to 4" >&2; exit 1; }
