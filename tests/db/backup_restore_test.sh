...
[[ "${source_schema}" == "18" ]] || { echo "ERROR: backup source expected latest schema 18, got ${source_schema}" >&2; exit 1; }
...
grep -Fq '"schema_version": 18,' "${backup_dir}/metadata.json"
...
[[ "${restored_schema}" == 18 ]] || { echo "ERROR: restored schema expected 18, got ${restored_schema}" >&2; exit 1; }
