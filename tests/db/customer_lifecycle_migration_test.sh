...
up="${repo_root}/db/migrations/0004_customer_lifecycle_foundation.up.sql"
down="${repo_root}/db/migrations/0004_customer_lifecycle_foundation.down.sql"

[[ -f "${up}" ]] || { echo "ERROR: missing 0004_customer_lifecycle_foundation.up.sql" >&2; exit 1; }
[[ -f "${down}" ]] || { echo "ERROR: missing 0004_customer_lifecycle_foundation.down.sql" >&2; exit 1; }

grep -Fqx -- '-- pvnaive:migration-version 0004' "${up}"
grep -Fqx -- '-- pvnaive:transactional true' "${up}"
grep -Fqx -- '-- pvnaive:destructive false' "${up}"
grep -Fqx -- '-- pvnaive:migration-version 0004' "${down}"
...
CREATE ROLE pvnaive_app LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT NOREPLICATION NOBYPASSRLS PASSWORD 'pvnaive-customer-ci-only';
...
SELECT
  ...
  EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='pvnaive' AND table_name='users' AND column_name='revision') || '|' ||
  EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='pvnaive' AND table_name='plans' AND column_name='revision') || '|' ||
...
[[ "${contract}" == "true|true|true|true|true|true|true" || "${contract}" == "t|t|t|t|t|t|t" ]] || {
  echo "ERROR: customer lifecycle schema contract failed: ${contract}" >&2
  exit 1
}
...
VALUES (
  'a4000000-0000-0000-0000-000000000001', NULL, 'owner',
...
INSERT INTO pvnaive.auth_sessions (
  ...
  'b4000000-0000-0000-0000-000000000001', NULL,
  'a4000000-0000-0000-0000-000000000001', decode(repeat('14',32),'hex'),
  'c4000000-0000-0000-0000-000000000001', decode(repeat('24',32),'hex'),
  clock_timestamp() + interval '1 hour', clock_timestamp() + interval '12 hours',
  decode(repeat('34',32),'hex')
);" >/dev/null

PGPASSWORD='pvnaive-customer-ci-only' psql --no-psqlrc --set ON_ERROR_STOP=1 \
...
INSERT INTO pvnaive.users (
  id, tenant_id, username, display_name, status, created_by_actor_id
)
SELECT
  'd4000000-0000-0000-0000-000000000001', id, 'test1', 'Test One', 'active',
...
INSERT INTO pvnaive.plans (
  ...
  reset_interval_seconds, base_price_minor, currency
)
SELECT
  'e4000000-0000-0000-0000-000000000001', id, 'direct-50g-30d', 'Direct 50G 30D',
  'active', 'naive', 50000000000, 2592000, NULL, 0, 'USD'
FROM pvnaive.tenants WHERE tenant_type='system' AND slug='direct';
...
(
  'f4000000-0000-0000-0000-000000000001', 'test1-runtime-a', decode(repeat('44',32),'hex'),
  decode(repeat('54',16),'hex'), decode(repeat('64',12),'hex'), 'runtime-v1',
  'active', 'panel', 'a4000000-0000-0000-0000-000000000001', 'a4000000-0000-0000-0000-000000000001'
),
(
  'f4000000-0000-0000-0000-000000000002', 'test1-runtime-b', decode(repeat('45',32),'hex'),
  decode(repeat('55',16),'hex'), decode(repeat('65',12),'hex'), 'runtime-v1',
  'active', 'panel', 'a4000000-0000-0000-0000-000000000001', 'a4000000-0000-0000-0000-000000000001'
);

INSERT INTO pvnaive.service_terms (
  id, tenant_id, user_id, plan_id, quota_bytes, duration_seconds,
  reset_interval_seconds, start_policy, purchased_at, state
)
SELECT
  '14000000-0000-0000-0000-000000000001', u.tenant_id, u.id,
  'e4000000-0000-0000-0000-000000000001', 50000000000, 2592000,
  NULL, 'on_first_successful_connection', clock_timestamp(), 'pending'
FROM pvnaive.users AS u WHERE u.id='d4000000-0000-0000-0000-000000000001';

INSERT INTO pvnaive.user_runtime_credentials (
  tenant_id, user_id, service_term_id, runtime_credential_id, role
)
SELECT
  u.tenant_id, u.id, '14000000-0000-0000-0000-000000000001',
  'f4000000-0000-0000-0000-000000000001', 'primary'
FROM pvnaive.users AS u WHERE u.id='d4000000-0000-0000-000000000001';

COMMIT;
SQL

snapshot="$(PGPASSWORD='pvnaive-customer-ci-only' psql --no-psqlrc --set ON_ERROR_STOP=1 \
  --host "${PVNAIVE_DB_HOST}" --port "${PVNAIVE_DB_PORT}" --username pvnaive_app --dbname "${test_db}" \
...
[[ "${snapshot}" == "50000000000|2592000|on_first_successful_connection|pending|true|true" || "${snapshot}" == "50000000000|2592000|on_first_successful_connection|pending|t|t" ]] || {
  echo "ERROR: term snapshot/start-policy contract failed: ${snapshot}" >&2
  exit 1
}
...
SELECT
  u.tenant_id, u.id, '14000000-0000-0000-0000-000000000001',
  'f4000000-0000-0000-0000-000000000002', 'primary'
FROM pvnaive.users AS u WHERE u.id='d4000000-0000-0000-0000-000000000001';
COMMIT;
SQL
second_primary_rc=$?
set -e
[[ "${second_primary_rc}" -ne 0 ]] || { echo "ERROR: second active primary runtime binding was accepted" >&2; exit 1; }
...
SELECT
  u.tenant_id, u.id, '24000000-0000-0000-0000-000000000001',
  'f4000000-0000-0000-0000-000000000001', 'primary'
FROM pvnaive.users AS u WHERE u.id='d4000000-0000-0000-0000-000000000001';
COMMIT;
SQL
...
# Drop v6, then v5, then exercise the v4 lifecycle rollback itself.
...
remaining="$(psql_admin --dbname "${test_db}" --tuples-only --no-align --command \
  "SELECT (to_regclass('pvnaive.naive_runtime_credentials') IS NOT NULL) || '|' ||
          (to_regclass('pvnaive.service_terms') IS NULL) || '|' ||
          (SELECT COALESCE(MAX(version),0) FROM pvnaive.schema_migrations)")"
[[ "${remaining}" == "true|true|3" || "${remaining}" == "t|t|3" ]] || {
  echo "ERROR: v4 rollback did not preserve v3 cleanly: ${remaining}" >&2
  exit 1
}
