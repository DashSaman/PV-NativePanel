...
# The migrations above schema16 are additive for this fixture.
# Step back from the newest applied schema to 16, then prove immutable scheduler history still
# prevents the destructive schema16 rollback exactly as this regression test originally intended.
for want in $(seq "$((expected_schema - 1))" -1 16); do
