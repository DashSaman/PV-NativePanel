...
PVNAIVE_DB_ENV_FILE="${env_file}" bash "${repo_root}/scripts/db/set-expected-schema-version.sh" 15 >/dev/null
grep -Fqx 'PVNAIVE_EXPECTED_SCHEMA_VERSION=15' "${env_file}"
[[ "$(grep -c '^PVNAIVE_EXPECTED_SCHEMA_VERSION=' "${env_file}")" == "1" ]]

if PVNAIVE_DB_ENV_FILE="${env_file}" bash "${repo_root}/scripts/db/set-expected-schema-version.sh" 16 >/dev/null 2>&1; then
