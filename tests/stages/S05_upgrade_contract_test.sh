...
if PVNAIVE_DB_ENV_FILE="${setter_env}" bash "${expected_schema_setter}" 12 >/dev/null 2>&1; then
  echo 'ERROR: expected schema setter accepted unsupported schema version 12' >&2
  exit 1
fi
