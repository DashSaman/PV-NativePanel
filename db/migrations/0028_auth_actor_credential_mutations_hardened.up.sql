...
-- Self-service actor credential mutations, hardened edition. The deployed
-- 0027 lineage created these SECURITY DEFINER functions without an
-- authentication-context guard, so any session that could reach pvnaive_app
-- could mutate another actor's password/profile by supplying an arbitrary
-- p_actor_id. This migration replaces both functions with guarded versions
-- that require a valid request context whose actor id matches the target
-- actor (defence in depth on top of the API layer).
